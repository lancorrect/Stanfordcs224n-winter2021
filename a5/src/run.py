...
if args.function == 'pretrain':  # 预训练模型
    ...
    tconf = trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=6e-3,
                                  lr_decay=True, warmup_tokens=512*20, numworks=4,
                                  final_tokens=200*len(pretrain_dataset)*block_size)
    trainer_our = trainer.Trainer(model_our, pretrain_dataset, None,tconf)
    trainer_our.train()
    torch.save(model_our.state_dict(), args.writing_params_path)
elif args.function == 'finetune':  # 微调模型
    assert args.writing_params_path is not None
    assert args.finetune_corpus_path is not None
    # TODO [part c] [part f]:
    # - Given:
    #     1. A finetuning corpus specified in args.finetune_corpus_path 微调语料库的路径
    #     2. A path args.reading_params_path containing pretrained model
    #         parameters, or None if finetuning without a pretrained model 预训练参数路径，没有则None
    #     3. An output path args.writing_params_path for the model parameters 输出路径
    # - Goals:
    ...
    #         final_tokens=200*len(pretrain_dataset)*block_size
    #         num_workers=4
    finetune_text = open(args.finetune_corpus_path).read()  # 读入需要微调的数据
    finetune_dataset = dataset.NameDataset(pretrain_dataset, finetune_text)  # 用预训练的词典等参数来encode微调数据集
    if args.reading_params_path is not None:  # 如果有预训练模型的话
        tconf = trainer.TrainerConfig(max_epochs=10, batch_size=256, learning_rate=6e-4,
                                      lr_decay=True, warmup_tokens=512*20, num_workers=0,
                                      final_tokens=200*len(pretrain_dataset)*block_size)
        # 载入模型
        model_our.load_state_dict(torch.load(args.reading_params_path,map_location='cuda:0'))
        trainer_our = trainer.Trainer(model_our, finetune_dataset, None, tconf)
        trainer_our.train()
    else:  # 如果没有预训练模型
    ...
    with open(args.outputs_path, 'w') as fout:
        ...
        for line in tqdm(open(args.eval_corpus_path)):
            ...
            pred = utils.sample(model_our, x, 32, sample=False)[0]
            completion = ''.join([pretrain_dataset.itos[int(i)] for i in pred])
            pred = completion.split('⁇')[1]  # 之前把每行数据都分割成了prefix+masked_content+suffix，这里取得是masked_content
