...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.key = nn.Linear(config.n_embd, config.n_embd)  # key权重
        self.query = nn.Linear(config.n_embd, config.n_embd)  # query权重
        self.value = nn.Linear(config.n_embd, config.n_embd)  # value权重
        # regularization
        ...
        # register_buffer是向模块添加永久缓冲区，应该就是在内存中保存一个常量，在模型加载和保存的时候可以写入和读出
        # torch.tril保持输入向量的下三角部分，其余部分为0
        self.register_buffer("mask", torch.tril(torch.ones(config.block_size, config.block_size))
                                     .view(1, 1, config.block_size, config.block_size))
        self.n_head = config.n_head

    def forward(self, x, layer_past=None):
        B, T, C = x.size()  # B是batch_size, T是单词数量，C是单词维度

        # calculate query, key, values for all heads in batch and move head forward to be the batch dim
        # nh表示头数，hs表示每个头中有多少单词维度
        k = self.key(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        q = self.query(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        v = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)

        # causal self-attention; Self-attend: (B, nh, T, hs) x (B, nh, hs, T) -> (B, nh, T, T)
        att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))  # 前面的transpose相当于是转置
        # 将mask中值为0的位置挑选出来，在att的相应位置上改为 - 1e10
...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        att = torch.matmul(self.relu(att), self.w2) + self.b2  # (B, nh, T, T)
        att = att.masked_fill(self.mask[:, :, :T, :T] == 0, -1e10)
        att = F.softmax(att, dim=-1)
        att = self.attn_drop(att)
        v = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1,2)  # (B, nh, T, hs)
        y = att @ v # (B, nh, T, T) x (B, nh, T, hs)  @表示矩阵意义上的相乘
        y = y.transpose(1, 2).contiguous().view(B, T, C)  # re-assemble all head outputs side by side
