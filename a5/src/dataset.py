...
class CharCorruptionDataset(Dataset):
    def __init__(self, data, block_size):
        ...
        chars.insert(0, self.MASK_CHAR)  # 插入到列表第一个位置上
        ...
        self.data = data.split('\n')  # 每句话是一个字符串，整个文档构成了一个列表，每个句子是一个元素
    ...
    def __getitem__(self, idx):
        ...
        if truncate_length > len(document):
            truncate_document = document
        else:
            truncate_document = document[:truncate_length]

        # 2 三个子串，prefix, masked_content, suffix
        mask_length = abs(int(random.normalvariate(int(len(truncate_document)/4), 2)))
        mask_length = mask_length if mask_length < len(truncate_document) else len(truncate_document)
        # print(len(truncate_document), mask_length)
        if mask_length == len(truncate_document):
            mask_start = 0
        elif len(truncate_document)-mask_length == 1 or len(truncate_document)-mask_length == 2:
            mask_start = 1
        else:
            mask_start = random.randint(0, (len(truncate_document)-mask_length-2))
        prefix = truncate_document[:mask_start]
        mask_content = truncate_document[mask_start:mask_start+mask_length]
        suffix = truncate_document[mask_start+mask_length:]

        # 3  prefix??suffix??masked_content[pads]
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR
        mask_content = "".join([self.MASK_CHAR for i in range(len(mask_content))])
        masked_string += str(mask_content)
