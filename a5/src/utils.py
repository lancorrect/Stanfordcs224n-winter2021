...
def sample(model, x, steps, temperature=1.0, sample=False, top_k=None):
    ...
    for k in range(steps):
        x_cond = x if x.size(1) <= block_size else x[:, -block_size:] # crop context if needed截取文本
        logits, _ = model(x_cond)  # 模型预测
        ...
        probs = F.softmax(logits, dim=-1)  # 每个单词的概率
        # sample from the distribution or take the most likely
        if sample:
            ix = torch.multinomial(probs, num_samples=1)
        else:
            _, ix = torch.topk(probs, k=1, dim=-1)  # 取概率最大的索引，k=1代表取排序好的第一个元素，dim=-1表示从原矩阵最后一维获取元素
...
def evaluate_places(filepath, predicted_places):
  ...
  with open(filepath) as fin:
    ...
    if len(lines[0]) == 1:
      ...
      return (0,0)
    true_places = [x[1] for x in lines]  # 出生地列表
