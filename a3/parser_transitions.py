...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'LA':
            top = self.stack.pop()
            pre = self.stack.pop()
            self.dependencies.append(tuple([top, pre]))
        ...
        if transition == 'RA':
            top = self.stack.pop()
            pre = self.stack.pop()
            self.dependencies.append(tuple([pre, top]))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parses = partial_parses
    num_parses = len(partial_parses)
    index = 0
    while num_parses > 0 :
        if num_parses - batch_size >= 0:
            minibatch = [unfinished_parses[i] for i in range(index, index+batch_size)]
        else:
            minibatch = [unfinished_parses[i] for i in range(index, len(unfinished_parses))]
        index += batch_size
        num_parses -= batch_size
        while minibatch:
            transitions = model.predict(minibatch)
            for j in range(len(minibatch)):
                # parse_step没有返回dependencies，最开始写的时候忘记返回的值是什么了，而且一定要看清楚并看懂伪代码
                minibatch[j].parse_step(transitions[j])
            # 后面的if条件不能是and，因为算法是先把buffer中的word都shift到stack中，然后再进行LA和RA操作。
            # 如果用and条件，则shift完buffer就空了，就无法把还没有进行LA和RA操作的句子放到循环变量中了
