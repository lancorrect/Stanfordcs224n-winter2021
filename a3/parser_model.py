...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = []
        for word in w:
            # index_select是一个挑元素的函数，第一个参数是被挑的对象，第二个是按照什么维度挑，0是按照行挑出来，1是按照列挑出来
            # 最后一个参数是索引，如果是按行挑就是挑选哪几行，如果是按列挑就是挑选哪几列
            x.append(torch.index_select(self.embeddings, 0, word))
        x = torch.tensor([item.detach().numpy() for item in x])  # 这里有个坑，在转换list到tensor时，需要此行代码
        x = x.view(-1, self.n_features*self.embed_size)  # x维度调整成(bath_size,n_features*embed_size)大小
    ...
    def forward(self, w):
        ...
        h = torch.matmul(w_emb, self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        relu = nn.ReLU()
