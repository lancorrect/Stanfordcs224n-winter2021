...
def train_for_epoch(parser, train_data, dev_data, optimizer, loss_func, batch_size):
    ...
    n_minibatches = math.ceil(len(train_data) / batch_size)  # 迭代次数
    ...
    with tqdm(total=(n_minibatches)) as prog:
        for i, (train_x, train_y) in enumerate(minibatches(train_data, batch_size)):
            ...
            loss_temp = loss_func(logits, train_y)
            loss_temp.backward()
            optimizer.step()
            loss += loss_temp
            ...
            ### END YOUR CODE
            prog.update(1)
            loss_meter.update(loss.item())  # loss_meter是AverageMeter类的实例，该类的作用是管理一些变量的更新
